...
extension NIMCreateTeamOption: NimDataConvertProtrol {
    func toDic() -> [String : Any]? {
        if var jsonObject = self.yx_modelToJSONObject() as? [String : Any] {
            ...
            jsonObject["extensionUpdateMode"] = FLT_NIMTeamUpdateClientCustomMode.convert(updateClientCustomMode)?.rawValue
            return jsonObject
        }
        return nil
    }
    ...
    static func fromDic(_ json: [String : Any]) -> Any? {
        if let model = NIMCreateTeamOption.yx_model(with: json) {
            if let teamType = json["teamType"] as? String, let type = FLT_NIMTeamType.init(rawValue: teamType) {
                model.type = type.convertNIMTeamType()
            }
            if let verifyType = json["verifyType"] as? String, let type = FLT_NIMTeamJoinMode.init(rawValue: verifyType) {
...
extension NIMTeam: NimDataConvertProtrol {
    ...
    dynamic var flt_type: FLT_NIMTeamMemberType? {
        get {
            if let T = objc_getAssociatedObject(self, UnsafeRawPointer.init(bitPattern: "flt_type".hashValue)!) as? FLT_NIMTeamMemberType {
                return T
            }else {
                return nil
            }
        ...
        set {
            objc_setAssociatedObject(self, UnsafeRawPointer.init(bitPattern: "flt_type".hashValue)!, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    ...
    func toDic() -> [String : Any]? {
        if var jsonObject = self.yx_modelToJSONObject() as? [String : Any]  {
            jsonObject["verifyType"] = FLT_NIMTeamJoinMode.convert(joinMode)?.rawValue
            jsonObject["messageNotifyType"] = FLT_NIMTeamNotifyState.convert(notifyStateForNewMsg)?.rawValue
            jsonObject["teamInviteMode"] = FLT_NIMTeamInviteMode.convert(inviteMode)?.rawValue
            jsonObject["teamBeInviteModeEnum"] = FLT_NIMTeamBeInviteMode.convert(beInviteMode)?.rawValue
            jsonObject["teamUpdateMode"] = FLT_NIMTeamUpdateInfoMode.convert(updateInfoMode)?.rawValue
            jsonObject["teamExtensionUpdateMode"] = FLT_NIMTeamUpdateClientCustomMode.convert(updateClientCustomMode)?.rawValue
            jsonObject["type"] = FLT_NIMTeamType.convert(type)?.rawValue
            jsonObject["isAllMute"] = self.inAllMuteMode()
            jsonObject["muteMode"] = self.inAllMuteMode() ? "muteAll" : "cancel"
            return jsonObject
        }
        return nil
    }
    ...
    static func fromDic(_ json: [String : Any]) -> Any? {
        if let model = NIMTeam.yx_model(with: json) {
            if let verifyType = json["verifyType"] as? String, let verify = FLT_NIMTeamJoinMode(rawValue: verifyType) {
            ...
            if let teamExtensionUpdateMode = json["teamExtensionUpdateMode"] as? String,
               let extensionUpdateMode = FLT_NIMTeamUpdateClientCustomMode(rawValue: teamExtensionUpdateMode){
...
extension NIMTeamMember: NimDataConvertProtrol {
    ...
    func toDic() -> [String : Any]? {
        if var jsonObject = self.yx_modelToJSONObject() as? [String : Any]  {
            ...
            jsonObject["joinTime"] = Int.init(createTime)
            if customInfo != nil,
    ...
    static func fromDic(_ json: [String : Any]) -> Any? {
        if let model = NIMTeamMember.yx_model(with: json) {
            if let memberType = json["type"] as? String, let mType = FLT_NIMTeamMemberType(rawValue: memberType) {
