//
//  FLTMessageService.swift
//  nim_core
//
//  Created by chenyu on 2021/8/11.
//
...
//    }
    ...
    func serviceName() -> String {
        return ServiceType.MessageService.rawValue
    }
    
    func onMethodCalled(_ method: String, _ arguments: [String : Any], _ resultCallback: ResultCallback) {
        
        if let chatExtendService = nimCore?.getService(ServiceType.ChatroomService.rawValue) as? FLTChatExtendService,
           chatExtendService.checkMethod(method) {
            chatExtendService.onMethodCalled(method, arguments, resultCallback)
            return
        }
        
        if let sessionService = nimCore?.getService(ServiceType.SessionService.rawValue) as? FLTSessionService,
        ...
        case MessageType.DownloadAttachment.rawValue:
            if let message = NIMMessage.convertToMessage(arguments), let sessionId = message.session?.sessionId, let sessionType = message.session?.sessionType  {
                if let databaseMessage = NIMSDK.shared().conversationManager.messages(in: NIMSession.init(sessionId, type: sessionType), messageIds: [message.messageId])?.first {
                    fetchMessageAttachment(databaseMessage, resultCallback)
                }else {
                    resultCallback.result(NimResult.error("resend message not exist").toDic() as Any)
                }
            }
    ...
    private func voiceToText(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        ...
        if let message = NIMMessage.convertToMessage(arguments), let attachment = message.messageObject as? NIMAudioObject {
            let option = NIMAudioToTextOption()
            if let dic = arguments["messageAttachment"] as? [String : Any], let path = dic["path"] as? String{
    ...
    private func sendMessage(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        ...
        if let message = NIMMessage.convertToMessage(arguments),
           ...
           let sessionType = message.sessionType, let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            ...
            do {
                if resend == false {
                    ...
                    if let resendMessage = NIMSDK.shared().conversationManager.messages(in: NIMSession.init(sessionId, type: type), messageIds: [message.messageId])?.first {
    ...
    private func createMessage(_ arguments: [String : Any]) -> (message: NIMMessage, session: NIMSession)? {
        var sessionId = ""
        ...
        if let messageTypeValue = arguments["messageType"] as? String,
           let messageType = try? NIMMessageType.getType(messageTypeValue) {
            ...
            case NIMMessageType.location:
                ...
                return createLocationMessage(sessionId, sessionType, attatchment?["lng"] as? Double, attatchment?["lat"] as? Double, attatchment?["title"] as? String)
    ...
    private func createTextMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ text: String?) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func createImageMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ attachment: [String : Any]?) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func createAudioMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ attachment: [String : Any]?) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func createVideoMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ attachment: [String : Any]?) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func createFileMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ attachment: [String : Any]?) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func createLocationMessage(_ sessionId: String?,
                                       ...
                                       _ longitude: Double?,
                                       _ latitude: Double?,
                                       _ address: String?) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func createTipMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ text: String?) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func createCustomMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ data: [String : Any]?)  -> (message: NIMMessage, session: NIMSession)? {
        if let sid = sessionId {
            let message = NIMMessage()
    ...
    private func createRobotMessage(_ sessionId: String?, _ sessionType: NIMSessionType, _ arguments:[String : Any]) -> (message: NIMMessage, session: NIMSession)?{
    ...
    private func cancelFetchingMessageAttachment(_ message: NIMMessage, _ resultCallback: ResultCallback){
...
extension FLTMessageService : NIMChatManagerDelegate {
    ...
    func willSend(_ message: NIMMessage) {
        if message.session?.sessionType == .chatroom {
            notifyEvent(ServiceType.ChatroomService.rawValue, "onMessageStatusChanged", message.toDic())
        } else {
    ...
    func send(_ message: NIMMessage, progress: Float) {
        notifyEvent(serviceName(), "onAttachmentProgress", ["id":message.messageId, "progress":progress]);
    }
    ...
    func onRecvMessageReceipts(_ receipts: [NIMMessageReceipt]) {
        ...
        for r in receipts {
            if let s = r.session {
                let messages = NIMSDK.shared().conversationManager.messages(in: s, messageIds: [r.messageId])
                if messages != nil {
                    for m in messages! {
                        if m.session?.sessionType == .chatroom {
                            notifyEvent(ServiceType.ChatroomService.rawValue, "onMessageStatusChanged", m.toDic())
                        } else {
...
extension FLTMessageService {
    ...
    func queryMessageList(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let sessionType = try? NIMSessionType.getType(arguments["sessionType"] as? String),
           let account = arguments["account"] as? String {
            ...
            let session = NIMSession.init(account, type: sessionType)
            if let ret = NIMSDK.shared().conversationManager.messages(in: session, message: nil, limit: limit) {
    ...
    func queryMessageListEx(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let messageArgs = arguments["message"] as? [String : Any],
           ...
           let sessionType = message.sessionType, let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            let session = NIMSession(sessionId, type: type)
            if let ret = NIMSDK.shared().conversationManager.messages(in: session, message: message, limit: arguments["limit"] as! Int) {
                let messageList = ret.map { message in
                    message.toDic()
                }
                let result = NimResult(["messageList" : messageList], 0, nil)
    ...
    func queryLastMessage(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let sessionId = arguments["account"] as? String,
           ...
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            let session = NIMSession(sessionId, type: type)
            if let ret = NIMSDK.shared().conversationManager.messages(in: session, message: nil, limit: 1) {
                if ret.count > 0 {
    ...
    func queryMessageListByUuid(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let sessionId = arguments["sessionId"] as? String,
           ...
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            let session = NIMSession(sessionId, type: type)
            if let ret = NIMSDK.shared().conversationManager.messages(in: session, messageIds: arguments["uuidList"] as! [String]) {
                let messageList = ret.map { message in
                    message.toDic()
                }
                let result = NimResult(["messageList" : messageList], 0, nil)
    ...
    func deleteChattingHistory(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let messageDic = arguments["message"] as? [String: Any], let message = NIMMessage.convertToMessage(messageDic) {
            let option: NIMDeleteMessageOption = NIMDeleteMessageOption()
    ...
    func pullMessageHistoryExType(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let messageArgs = arguments["message"] as? [String : Any],
           ...
           let sessionType = message.sessionType, let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            ...
            option.order = arguments["direction"] as? Int == 1 ? NIMMessageSearchOrder.asc : NIMMessageSearchOrder.desc
    ...
    func clearServerHistory(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        let session = NIMSession.init(sessionId, type: sessionType2!)
        
        let option = NIMClearMessagesOption()
        option.removeRoam = sync
        
        NIMSDK.shared().conversationManager.deleteSelfRemoteSession(session, option: option) { error in
            if error != nil {
                let nserror = error! as NSError
                let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                resultCallback.result(result.toDic())
    ...
    func deleteMsgSelf(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let m = arguments["message"] as? [String : Any],
           ...
           let sessionType2 = try? NIMSessionType.getType(sessionType) {
            if let messages = NIMSDK.shared().conversationManager.messages(in: NIMSession.init(sessionId, type: sessionType2), messageIds: [message.messageId]),
               messages.count > 0 {
                let ext = arguments["ext"] as? String
                NIMSDK.shared().conversationManager.deleteMessage(fromServer: messages.first!, ext: ext) { error in
                    if error != nil {
                        let nserror = error! as NSError
                        let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                        resultCallback.result(result.toDic())
                    ...
                    for (session, value) in msgMap! {
                        for msg in value {
                            if var dic = msg.toDic() {
                                ...
                                dic["sessionType"] = FLT_NIMSessionType.convertFLTSessionType(session.sessionType)?.rawValue
                                messages.append(dic)
                            }
                        }
                    }
    ...
    func searchRoamingMsg(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        let session = NIMSession.init(otherAccid, type: NIMSessionType.P2P)
        ...
        NIMSDK.shared().conversationManager.retrieveServerMessages(session, option: option) { error, messages in
            if error != nil {
                let nserror = error! as NSError
                let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                resultCallback.result(result.toDic())
    ...
    func saveMessage(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let message = NIMMessage.convertToMessage(arguments),
           ...
           let sessionType = message.sessionType,
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            let session = NIMSession.init(sessionId, type: type)
            message.gArgument = arguments
            message.setValue(session, forKeyPath: #keyPath(NIMMessage.session))
    ...
    func updateMessage(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let message = NIMMessage.convertToMessage(arguments),
           ...
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            ...
            if let messages = NIMSDK.shared().conversationManager.messages(in: session, messageIds: [message.messageId]),
               messages.count > 0 {
    ...
    func messageTransportProgress(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let message = NIMMessage.convertToMessage(arguments),
           ...
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            ...
            resultCallback.result(NimResult.success(["transportProgress" : transportProgress]).toDic())
        } else {
            resultCallback.result(NimResult.error("create message error").toDic())
        }
    }
    ...
    func revokeMessage(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let message = NIMMessage.convertToMessage(arguments),
           ...
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            ...
            if let messages = NIMSDK.shared().conversationManager.messages(in: session, messageIds: [message.messageId]),
               messages.count > 0 {
                NIMSDK.shared().chatManager.revokeMessage(messages.first!, option: option) { error in
                    if error != nil {
                        let nserror = error! as NSError
                        let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                        resultCallback.result(result.toDic())
    ...
    func sendTeamMessageReceipts(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let message = NIMMessage.convertToMessage(arguments),
           ...
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            ...
            receipts.append(NIMMessageReceipt.init(message: message))
    ...
    func queryMessageReceiptDetail(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let message = NIMMessage.convertToMessage(arguments),
           ...
           let type = FLT_NIMSessionType(rawValue: sessionType)?.convertToNIMSessionType()  {
            ...
            NIMSDK.shared().chatManager.queryMessageReceiptDetail(message, accountSet: set) { error, receiptDetail in
                if error != nil {
                    let nserror = error! as NSError
                    let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                    resultCallback.result(result.toDic())
    ...
    func queryMySessionList(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        NIMSDK.shared().conversationManager.fetchServerSessions(option) { error, sessions, hasMore in
            if error != nil {
                let nserror = error! as NSError
                let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                resultCallback.result(result.toDic())
                ...
                if sessions != nil,
                   sessions!.count > 0 {
                    let sessionDics = sessions!.map { session in
                        return session.toDicEx()
                    }
                    resultCallback.result(NimResult.success(["mySessionList":["sessionList":sessionDics,"hasMore":hasMore]]).toDic())
                } else {
                    resultCallback.result(NimResult.success().toDic())
                }
            }
    ...
    func queryMySession(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let sessionId = arguments["sessionId"] as? String,
           ...
           let sessionType2 = try? NIMSessionType.getType(sessionType) {
            let session = NIMSession.init(sessionId, type: sessionType2)
            NIMSDK.shared().conversationManager.fetchServerSession(by: session) { error, recentSession in
                if error != nil {
                    let nserror = error! as NSError
                    let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                    resultCallback.result(result.toDic())
                } else {
                    resultCallback.result(NimResult.success(["recentSession":recentSession?.toDicEx()]).toDic())
                }
            }
        } else {
            resultCallback.result(NimResult.error("arguments invalid").toDic())
    ...
    func updateMySession(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let sessionId = arguments["sessionId"] as? String,
           ...
           let ext = arguments["ext"] as? String {
            let session = NIMSession.init(sessionId, type: sessionType2)
            NIMSDK.shared().conversationManager.updateServerSessionExt(ext, session: session) { error in
                if error != nil {
                    let nserror = error! as NSError
                    let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                    resultCallback.result(result.toDic())
...
extension FLTMessageService {
    ...
    func removeCollect(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let collects = arguments["collects"] as? [[String : Any]] {
            var infoList: [NIMCollectInfo] = [NIMCollectInfo]()
        ...
        NIMSDK.shared().chatExtendManager.queryCollect(options) { error, infos, totalCount in
            if error != nil {
                ...
                if infos != nil {
                    ...
                    resultCallback.result(NimResult.success(["totalCount":totalCount, "collects":collects]).toDic())
                } else {
    ...
    func addQuickComment(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        NIMSDK.shared().chatExtendManager.add(NIMQuickComment.fromDic(arguments), to: comment.message) { error in
            if error != nil {
                let nserror = error! as NSError
                let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                resultCallback.result(result.toDic())
                    ...
                    if (table != nil) {
                        for key in table!.keyEnumerator() {
                            if let msg = key as? NIMMessage {
                                let sessionType = FLT_NIMSessionType.convertFLTSessionType(msg.session!.sessionType)?.rawValue
                                let k = ["sessionType":sessionType, "fromAccount":msg.from, "toAccount":msg.sessionId, "time":Int.init(msg.timestamp), "serverId":Int.init(msg.serverID), "uuid":msg.messageId] as [String : Any]
                                //                                let modify = msg.
                                ...
                                let dic = ["key":k, "time":Int.init(time), "quickCommentList":quickCommentList] as [String : Any]
                                result.append(dic)
                            }
                        }
                    }
                    resultCallback.result(NimResult.success(["quickCommentOptionWrapperList":result]).toDic())
                }
            }
        } else {
            resultCallback.result(NimResult.error("arguments invalid").toDic())
           ...
           let sessionType = try? NIMSessionType.getType(sessionTypeValue) {
            ...
            NIMSDK.shared().chatExtendManager.addStickTopSession(params) { error, sessionInfo in
                if error != nil {
                    ...
                    if sessionInfo != nil {
                        resultCallback.result(NimResult.success(["stickTopSessionInfo":sessionInfo!.toDic()]).toDic())
                    } else {
                        resultCallback.result(NimResult.success().toDic())
                    }
                }
    ...
    func removeStickTopSession(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        NIMSDK.shared().chatExtendManager.removeStickTopSession(NIMStickTopSessionInfo.fromDic(arguments)) { error, sessionInfo in
            if error != nil {
                let nserror = error! as NSError
                let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                resultCallback.result(result.toDic())
    ...
    func updateStickTopSession(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        NIMSDK.shared().chatExtendManager.udpateStickTopSession(NIMStickTopSessionInfo.fromDic(arguments)) { error, sessionInfo in
            if error != nil {
                let nserror = error! as NSError
                let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                resultCallback.result(result.toDic())
    ...
    func queryStickTopSession(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        NIMSDK.shared().chatExtendManager.loadStickTopSessionInfos { error, dic in
            if error != nil {
                ...
                if dic != nil {
                    let infos = dic!.values.flatMap { info in
                        info.toDic()
                    }
    ...
    func loadRecentSessionsWithOptions(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        NIMSDK.shared().chatExtendManager.loadRecentSessions(with: NIMLoadRecentSessionsOptions.yx_model(with: arguments) ?? NIMLoadRecentSessionsOptions()) { error, sessions in
            if error != nil {
                let nserror = error! as NSError
                let result = NimResult(nil, NSNumber(value: nserror.code), nserror.description)
                resultCallback.result(result.toDic())
...
extension FLTMessageService: NIMChatExtendManagerDelegate {
    func onRecvQuickComment(_ comment: NIMQuickComment) {
        if let session = comment.message.session,
           let sessionType = FLT_NIMSessionType.convertFLTSessionType(session.sessionType)?.rawValue {
            let key = ["sessionType":sessionType, "fromAccount":comment.message.from, "toAccount":comment.message.sessionId, "time":Int.init(comment.message.timestamp), "serverId":Int.init(comment.message.serverID), "uuid":comment.message.messageId] as [String : Any]
            notifyEvent(serviceName(), "onQuickCommentAdd", ["key":key, "commentOption":comment.toDic()])
        }
    }
    ...
    func onRemove(_ comment: NIMQuickComment) {
        if let session = comment.message.session,
           let sessionType = FLT_NIMSessionType.convertFLTSessionType(session.sessionType)?.rawValue {
            let key = ["sessionType":sessionType, "fromAccount":comment.message.from, "toAccount":comment.message.sessionId, "time":Int.init(comment.message.timestamp), "serverId":Int.init(comment.message.serverID), "uuid":comment.message.messageId] as [String : Any]
            notifyEvent(serviceName(), "onQuickCommentRemove", ["key":key, "commentOption":comment.toDic()])
        }
    }
