...
//    }
    ...
    override func onInitialized() {
        ...
        NIMChatroomIndependentMode.registerRequestChatroomAddressesHandler { roomId, callback in
            self.notifyEvent(self.serviceName(), "getIndependentModeLinkAddress", ["roomId":roomId], result: { r in
    ...
    func serviceName() -> String {
        return ServiceType.ChatroomService.rawValue
    }
    ...
    func onMethodCalled(_ method: String, _ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        case ChatRoomType.CreateMessage.rawValue:
            if let messageService = nimCore?.getService(ServiceType.MessageService.rawValue) as? FLTMessageService {
        ...
        case ChatRoomType.FetchMessageHistory.rawValue: fetchMessageHistory(arguments, resultCallback)
        ...
        case ChatRoomType.DownloadAttachment.rawValue:
            if let messageService = nimCore?.getService(ServiceType.MessageService.rawValue) as? FLTMessageService {
                messageService.onMethodCalled(MessageType.DownloadAttachment.rawValue, arguments, resultCallback)
            }else {
                resultCallback.notImplemented()
            }
        
        //聊天室成员管理
        case ChatRoomType.FetchChatroomMembers.rawValue: fetchChatroomMembers(arguments, resultCallback)
        case ChatRoomType.FetchChatroomMembersByIds.rawValue: fetchChatroomMembersByIds(arguments, resultCallback)
        case ChatRoomType.UpdateMyChatroomMemberInfo.rawValue: updateMyChatroomMemberInfo(arguments, resultCallback)
        ...
        case ChatRoomType.UpdateMemberTempMute.rawValue: updateMemberTempMute(arguments, resultCallback)
        
        //聊天室队列
        case ChatRoomType.FetchChatroomQueue.rawValue: fetchChatroomQueue(arguments, resultCallback)
        case ChatRoomType.UpdateChatroomQueueObject.rawValue: updateChatroomQueueObject(arguments, resultCallback)
        case ChatRoomType.BatchUpdateChatroomQueueObject.rawValue: batchUpdateChatroomQueueObject(arguments, resultCallback)
        case ChatRoomType.RemoveChatroomQueueObject.rawValue: removeChatroomQueueObject(arguments, resultCallback)
    ...
    private func enterChatroom(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let request = NIMChatroomEnterRequest.fromDic(arguments) as? NIMChatroomEnterRequest else {
        ...
        NIMSDK.shared().chatroomManager.enterChatroom(request) { error, chatromm, member in
            weakSelf?.chatroomCallback(error, ["roomId":chatromm?.roomId, "roomInfo": chatromm?.toDic(), "member": member?.toDic(roomId: chatromm?.roomId)], resultCallback)
        }
    }
    ...
    private func fetchMessageHistory(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let roomId = getRoomId(arguments),
              ...
              let option = NIMHistoryMessageSearchOption.fromDic(optionJson) as? NIMHistoryMessageSearchOption else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
        
        weak var weakSelf = self
        NIMSDK.shared().chatroomManager.fetchMessageHistory(roomId, option: option) { error, messages in
        ...
        if let r = arguments["request"] as? [String : Any] {
            ...
            if let queueModificationLevel = r["queueModificationLevel"] as? String {
                if queueModificationLevel == "manager" {
                    info[NSNumber(value: NIMChatroomUpdateTag.queueModificationLevel.rawValue)] = NSNumber(booleanLiteral: true)
                } else {
                    info[NSNumber(value: NIMChatroomUpdateTag.queueModificationLevel.rawValue)] = NSNumber(booleanLiteral: false)
                }
            }
    ...
    //聊天室成员管理
    private func fetchChatroomMembers(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let roomId = getRoomId(arguments) else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
        ...
        if let lastMemberId = arguments["lastMemberAccount"] as? String {
            ...
            NIMSDK.shared().chatroomManager.fetchChatroomMembers(byIds: idRequest) { error, members in
                if let ns_error = error as NSError?{
    ...
    private func fetchChatroomMembersByIds(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let request = NIMChatroomMembersByIdsRequest.fromDic(arguments) as? NIMChatroomMembersByIdsRequest else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
    ...
    private func updateMyChatroomMemberInfo(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let roomId = getRoomId(arguments)  else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
        ...
        if let infoRequest = arguments["request"] as? [String: Any]{
            ...
            if let name = infoRequest["nickname"] as? String {
                updateInfo[NSNumber(integerLiteral: NIMChatroomMemberInfoUpdateTag.nick.rawValue)] = name
            }
            if let avatar = infoRequest["avatar"] as? String {
                updateInfo[NSNumber(integerLiteral: NIMChatroomMemberInfoUpdateTag.avatar.rawValue)] = avatar
            }
    ...
    private func updateMemberTempMute(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let options = arguments["options"] as? [String : Any] else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
    ...
    private func updateChatroomQueueObject(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let roomId = getRoomId(arguments)  else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
    ...
    private func batchUpdateChatroomQueueObject(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let roomId = getRoomId(arguments) else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
    ...
    private func removeChatroomQueueObject(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        guard let request = NIMChatroomQueueRemoveRequest.fromDic(arguments) as? NIMChatroomQueueRemoveRequest else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
...
extension FLTChatRoomService: NIMChatroomManagerDelegate {
    ...
    func chatroomBeKicked(_ result: NIMChatroomBeKickedResult) {
        notifyEvent(serviceName(), "onKickOut", ["roomId": result.roomId, "reason":FLT_NIMChatroomKickReason.convert(result.reason).rawValue, "extension": result.ext])
    ...
    func chatroom(_ roomId: String, connectionStateChanged state: NIMChatroomConnectionState) {
        // 缺少code
        notifyEvent(serviceName(), "onStatusChanged", ["roomId": roomId, "status":FLT_NIMChatroomConnectionState.convert(state)?.rawValue as Any])
