...
class FLTMessageService(
    ...
    private fun sendMessage(
        ...
        if (message == null) resultCallback.result(
            ...
            if (future == null) {
                ...
                resultCallback.result(
                    NimResult(code = -2, errorDetails = "send message error! message = ${getMessageInfo(message)}")
                )
            } else
                future.setCallback(object : RequestCallback<Void> {
                    ...
                    override fun onException(exception: Throwable?) {
                        ...
                        resultCallback.result(
                            NimResult(code = -2, errorDetails = "send message exception：message = ${getMessageInfo(message)} exception = $exception")
                        )
                    }
                })
        }
    ...
    private fun listMessageRequestCallback(resultCallback: ResultCallback<List<IMMessage>>,reversed:Boolean = false) =
    ...
    private suspend fun deleteSession(arguments: Map<String, *>):
        ...
        // 需要手动触发一下
        if (result.isSuccess && contact != null && (deleteTypeEnum == DeleteTypeEnum.LOCAL
            ...
            msgService.queryQuickComment(msgList)
                .setCallback(NimResultContinuationCallback(cont) { result ->
                    NimResult(
                        ...
                        data = result,
                        convert = { data ->
                            mapOf("quickCommentOptionWrapperList" to data.map { it.toMap() }
                                .toList())
                        }
                    )
                })
